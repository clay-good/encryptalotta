...
CII rendering, the embedded standalone-XML path, and profile identification ship today; the remaining deferred slice is **hybrid PDF/A-3 attachment extraction** (the `/EmbeddedFiles` walk shared with the §2.5 PAdES work — a user can paste the standalone CII XML today), documented in `SPEC-INTERNATIONAL.md` §2.7.
