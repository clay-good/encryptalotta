...
// ===================================================================
// Argon2 (RFC 9106) — derive through the real UI with small (fast) params.
// Expected tags were generated by the in-page implementation and verified
// byte-for-byte against the official RFC 9106 vectors and @noble/hashes in
// the build harness; here we assert the UI surfaces the right hex + PHC.
// Params m=64 KiB, t=2, p=1, 32-byte tag, salt "somesalt12345678" run in
// well under a second, so these stay inside the normal test timeout.
// ===================================================================
async function argon2Derive(page, { variant, password, salt, m, t, p, keyLen }) {
  await page.goto('/index.html'); await gotoTool(page, 'argon2');
  await page.selectOption('#argon2-variant', variant);
  await page.fill('#argon2-password', password);
  await page.fill('#argon2-salt', salt);
  await page.fill('#argon2-memory', String(m));
  await page.fill('#argon2-time', String(t));
  await page.fill('#argon2-parallelism', String(p));
  await page.fill('#argon2-keylen', String(keyLen));
  await page.click('#btn-argon2-derive');
  await page.waitForFunction(() => {
    const r = document.getElementById('argon2-results');
    return r && !r.classList.contains('hidden') && (r.textContent || '').length > 40;
  }, { timeout: 15_000 });
  return readResultText(page, '#argon2-results');
}
async function readResultText(page, sel) { return (await page.locator(sel).textContent()); }

test('argon2: Argon2id derives the expected hex + PHC string (RFC 9106 verified)', async ({ page }) => {
  const out = await argon2Derive(page, { variant: 'argon2id', password: 'password', salt: 'somesalt12345678', m: 64, t: 2, p: 1, keyLen: 32 });
  expect(out).toContain('d1dd9f4bfa15a408c1ae553f4c84aa83d73afcdb94a3b5b597e8eb201489b96b');
  expect(out).toContain('$argon2id$v=19$m=64,t=2,p=1$c29tZXNhbHQxMjM0NTY3OA$0d2fS/oVpAjBrlU/TISqg9c6/NuUo7W1l+jrIBSJuWs');
});

test('argon2: Argon2i derives the expected hex (RFC 9106 verified)', async ({ page }) => {
  const out = await argon2Derive(page, { variant: 'argon2i', password: 'password', salt: 'somesalt12345678', m: 64, t: 2, p: 1, keyLen: 32 });
  expect(out).toContain('1e0e7a24cacf7853d7041b2942b6d422ae26e49d955cbff2014822cf2bf2977b');
  expect(out).toContain('$argon2i$v=19$');
});

test('argon2: Argon2d derives the expected hex (RFC 9106 verified)', async ({ page }) => {
  const out = await argon2Derive(page, { variant: 'argon2d', password: 'password', salt: 'somesalt12345678', m: 64, t: 2, p: 1, keyLen: 32 });
  expect(out).toContain('745ff6dfbff861c49746bb3c31e13f7cfd191dfc1809c4686af7d1e97820e1dd');
});

test('argon2: rejects a salt shorter than 8 bytes', async ({ page }) => {
  await page.goto('/index.html'); await gotoTool(page, 'argon2');
  await page.fill('#argon2-password', 'pw');
  await page.fill('#argon2-salt', 'short');
  await page.fill('#argon2-memory', '64');
  await page.click('#btn-argon2-derive');
  await expect(page.locator('#argon2-results')).toContainText('8', { timeout: 10_000 });
  // The known-good tag must NOT appear for an invalid run.
  await expect(page.locator('#argon2-results')).not.toContainText('d1dd9f4bfa15a408');
});

test('argon2: OWASP preset fills m=19456, t=2, p=1', async ({ page }) => {
  await page.goto('/index.html'); await gotoTool(page, 'argon2');
  await page.selectOption('#argon2-preset', 'owasp');
  await expect(page.locator('#argon2-memory')).toHaveValue('19456');
  await expect(page.locator('#argon2-time')).toHaveValue('2');
  await expect(page.locator('#argon2-parallelism')).toHaveValue('1');
  // Editing a parameter resets the preset select back to custom.
  await page.fill('#argon2-time', '3');
  await expect(page.locator('#argon2-preset')).toHaveValue('custom');
});
